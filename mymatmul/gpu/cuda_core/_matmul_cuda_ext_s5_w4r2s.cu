...
/*
 * Stage 5 W4R2S: s5_w4r2 + GROUP_M=2 block-index swizzle.
 *
 * Same kernel as s5_w4r2 (128 threads, 2×2 inter-warp) but uses a 1D grid
 * with GROUP_M=2 swizzle so consecutive block pairs cover the same N column
 * (adjacent M stripes).  Co-resident SM blocks therefore share the same B
 * tile column in L2, recovering the B-tile reuse that w4r gets from its
 * larger BM=256 tile.
 *
 * Grid: 1D, total = grid_m * grid_n blocks.  GROUP_M=4 swizzle (M,N power-of-2):
 *   quarter = pid >> 2
 *   m_id    = (quarter / GN) * 4 + (pid & 3)   — 4 rows before advancing N
 *   n_id    = quarter & (GN - 1)               — mask ok since GN is power of 2
 * No min() / edge-case needed; grid_m always divisible by 4 under power-of-2.
 *
 * All other details (register double-buffering, intra-warp layout) identical
 * to s5_w4r2.
 */

template <int BM, int BN, int BK, int UNROLL>
__device__ __forceinline__ void matmul_s5_w4r2s_impl(
    ...
    const int GN       = N / BN;           // power-of-2 assumed: exact
    const int pid      = blockIdx.x;
    const int quarter  = pid >> 2;
    const int m_id     = (quarter / GN) * 4 + (pid & 3);   // group_id*4 + row_in_group
    const int n_id     = quarter & (GN - 1);                // quarter % GN, mask ok since pow2
...
void matmul_cuda_s5_w4r2s_bm##BM_##_bn##BN_##_bk##BK_##_u##U_(                      \
    const float* __restrict__ A, const float* __restrict__ B,                      \
    float* __restrict__ C, int M, int K, int N) {                                  \
    matmul_s5_w4r2s_impl<BM_, BN_, BK_, U_>(A, B, C, M, K, N);                      \
}
