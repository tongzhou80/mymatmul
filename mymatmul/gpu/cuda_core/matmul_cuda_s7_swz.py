...
_TEMPLATE_CU = os.path.join(
    ...
    "_matmul_cuda_ext_s7_swz_template.cu",
)
...
def _kname(bm, bn, bk, u, nw):
    return f"matmul_cuda_s7_swz_bm{bm}_bn{bn}_bk{bk}_u{u}_nw{nw}"
...
def matmul_s7_swz(A, B):
    M, K = A.shape
    _, N = B.shape
    key  = (M, N, K)
    if key not in _best:
        cfgs = _configs(M, N, K)
        print(f"[s7_swz] autotuning {M}x{K}x{N} over {len(cfgs)} configs ...")
        _best[key] = _tune(M, N, K)
        bm, bn, bk, u, nw = _best[key]
        print(f"[s7_swz] best: BM={bm} BN={bn} BK={bk} U={u} NW={nw}")
