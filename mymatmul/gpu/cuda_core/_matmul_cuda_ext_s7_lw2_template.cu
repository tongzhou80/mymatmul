...
/*
 * Stage 7 lw2: s7 with a 2×16 intra-warp layout (LWARP_M=2, LWARP_N=16).
 *
 * vs. s7's 4×8 layout:
 *   - B tile loaded as float2 (not float4) → 2× more load instructions but
 *     half the bank conflicts on B (2-way vs 4-way per warp-row).
 *   - TM doubles, TN halves (same total accumulator registers).
 *
...
__device__ __forceinline__ void matmul_s7_lw2_impl(
    ...
    constexpr int LWARP_N = 16;   // 2×16 intra-warp layout
    constexpr int LWARP_M = 2;
    ...
    const int intra_lty = tiw / LWARP_N;   // 0..1
    const int intra_ltx = tiw % LWARP_N;   // 0..15
...
#define COMPUTE_TILE(buf_)                                                              \
    do {                                                                                \
        ...
        for (int _j = 0; _j < TN / 2; _j++)                                           \
            _bv[0][_j] = *reinterpret_cast<const float2*>(                             \
                &B_shared[(buf_)][0]                                                   \
                          [warp_col * WARP_TILE_N + 2 * intra_ltx                     \
                           + _j * (2 * LWARP_N)]);                                     \
        ...
        for (int _kk = 0; _kk < BK - 1; _kk++) {                                     \
            ...
            _Pragma("unroll")                                                           \
            for (int _j = 0; _j < TN / 2; _j++)                                      \
                _bv[_nxt][_j] = *reinterpret_cast<const float2*>(                     \
                    &B_shared[(buf_)][_kk + 1]                                         \
                              [warp_col * WARP_TILE_N + 2 * intra_ltx                 \
                               + _j * (2 * LWARP_N)]);                                 \
            _Pragma("unroll")                                                           \
            for (int _j = 0; _j < TN / 2; _j++) {                                    \
                _Pragma("unroll")                                                       \
                for (int _i = 0; _i < TM; _i++) {                                    \
                    acc[_i][_j * 2 + 0] += _a[_cur][_i] * _bv[_cur][_j].x;          \
                    acc[_i][_j * 2 + 1] += _a[_cur][_i] * _bv[_cur][_j].y;          \
                }                                                                       \
            }                                                                           \
        }                                                                               \
        {                                                                               \
            ...
            for (int _j = 0; _j < TN / 2; _j++) {                                    \
                _Pragma("unroll")                                                       \
                for (int _i = 0; _i < TM; _i++) {                                    \
                    acc[_i][_j * 2 + 0] += _a[_last][_i] * _bv[_last][_j].x;        \
                    acc[_i][_j * 2 + 1] += _a[_last][_i] * _bv[_last][_j].y;        \
                }                                                                       \
            }                                                                           \
        }                                                                               \
    } while (0)
...
#undef COMPUTE_TILE
    ...
    for (int i = 0; i < TM; i++)
        ...
        for (int j = 0; j < TN / 2; j++) {
            ...
            const int gc = block_col + warp_col * WARP_TILE_N + 2 * intra_ltx + j * (2 * LWARP_N);
            *reinterpret_cast<float2*>(&C[gr * N + gc]) =
                make_float2(acc[i][j*2+0], acc[i][j*2+1]);
        }
}

#define MAKE_LAUNCHER(BM_, BN_, BK_, U_, NW_)                                      \
extern "C" __global__ __launch_bounds__(NW_ * 32)                                  \
void matmul_cuda_s7_lw2_bm##BM_##_bn##BN_##_bk##BK_##_u##U_##_nw##NW_(           \
    const float* __restrict__ A, const float* __restrict__ B,                      \
    float* __restrict__ C) {                                                        \
    matmul_s7_lw2_impl<BM_, BN_, BK_, U_, NW_>(A, B, C);                           \
}
