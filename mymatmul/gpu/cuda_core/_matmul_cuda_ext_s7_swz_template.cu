...
__device__ __forceinline__ void matmul_s7_swz_impl(
    ...
    // Swizzle-by-2: pairs of consecutive CTAs cover two adjacent rows in the
    // same column before advancing to the next column.
    //   CTA 0 → (row=0,col=0), CTA 1 → (row=1,col=0),
    //   CTA 2 → (row=0,col=1), CTA 3 → (row=1,col=1), …
    // GN is a compile-time constant → % and / are optimised by the compiler.
    constexpr int GN    = N_VAL / BN;
    const int pid       = blockIdx.x;
    const int half      = pid >> 1;
    const int m_id      = (half / GN) * 2 + (pid & 1);
    const int n_id      = half % GN;
...
void matmul_cuda_s7_swz_bm##BM_##_bn##BN_##_bk##BK_##_u##U_##_nw##NW_(               \
    const float* __restrict__ A, const float* __restrict__ B,                      \
    float* __restrict__ C) {                                                        \
    matmul_s7_swz_impl<BM_, BN_, BK_, U_, NW_>(A, B, C);                               \
}
