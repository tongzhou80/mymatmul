"""Stage 5 PTX: s5 with raw PTX cp.async.cg.shared.global.L2::128B instead of __pipeline_memcpy_async."""

import time
import torch
from .._pycuda_loader import launch_matmul, get_module

_EXT = "_matmul_cuda_ext_s5_ptx"
...
def _kname(bm, bn, bk, u):
    return f"matmul_cuda_s5_ptx_bm{bm}_bn{bn}_bk{bk}_u{u}"
...
def _tune(M, N, K):
    ...
    for idx, cfg in enumerate(_CONFIGS):
        ...
        if bk == 16 and u == 16:
            print(f"  [{idx+1:2d}/{n}] BM={bm:3d} BN={bn:3d} BK={bk:2d} U={u:2d}   {gflops:6.1f} TFLOPS")
...
def matmul_s5_ptx_bm256_bn128_bk32_u16(A, B):
...
def matmul_s5_ptx(A, B):
    M, K = A.shape
    _, N = B.shape
    key  = (M, N, K)
    if key not in _best:
        print(f"[s5_ptx] autotuning {M}x{K}x{N} over {len(_CONFIGS)} configs ...")
        _best[key] = _tune(M, N, K)
        bm, bn, bk, u = _best[key]
        print(f"[s5_ptx] best: BM={bm} BN={bn} BK={bk} U={u}")
