...
/*
 * Stage 7: s6 kernel with M, N, K baked as compile-time constants.
 *
...
__device__ __forceinline__ void matmul_s7_impl(
...
void matmul_cuda_s7_bm##BM_##_bn##BN_##_bk##BK_##_u##U_##_nw##NW_(               \
    const float* __restrict__ A, const float* __restrict__ B,                      \
    float* __restrict__ C) {                                                        \
    matmul_s7_impl<BM_, BN_, BK_, U_, NW_>(A, B, C);                               \
}
