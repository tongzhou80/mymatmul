 ...
 *
 * Pure cluster-skeleton experiment: identical to h2_s7 in every line of code
 * except the launcher carries __cluster_dims__(1, 2, 1). No DSMEM, no shared
 * loads, no cluster.sync. Goal: measure the *baseline* overhead of launching
 * CTAs in 2-CTA clusters so we can separate "cluster cost" from "cluster
 * sharing cost" in future experiments.
 *
 * Constraints from __cluster_dims__(1, 2, 1):
 *   - gridDim.y must be a multiple of 2 (i.e., M/BM must be even)
 */
...
void matmul_h4_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_##_ns##NS_(                \
    const __nv_bfloat16* __restrict__ A,                                         \
    const __nv_bfloat16* __restrict__ B,                                         \
    __nv_bfloat16* __restrict__ C,                                               \
    int M, int K, int N)                                                         \
