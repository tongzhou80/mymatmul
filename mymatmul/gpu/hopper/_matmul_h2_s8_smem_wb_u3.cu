...
/*
 * h2_s8_smem_wb_u3: h2_s8_smem_wb specialised for NS=3 and K-divisible-by-192.
 *
 * The K-loop body has period NS=3 in the slot indices (cur, nxt). Unrolling
 * by 3 turns them into compile-time constants 0/1/2, eliminating the
 *   - `k % 3` magic-number sequence (5 ops/iter)
 *   - `slot * stage_stride` runtime add (compiler folds to immediate offsets)
 * for both COMPUTE_TILE (wgmma SMEM descriptors) and LOAD_TILE (cp.async
 * SMEM destinations).
 *
 * Sized to K=8192 (num_tiles = 128): main loop runs 126 = 3 × 42 iters, so
 * (num_tiles - (NS-1)) is exactly divisible by 3 — no remainder loop needed.
 * (Works for any K where (K/BK - 2) % 3 == 0; assert at launch.)
 */
...
__device__ __forceinline__ void h2_s8_smem_wb_u3_impl(
    ...
    //
    // Pipeline loop (same structure will be reused for TMA backend in h2_s8_smem_wb_u3):
    //
...
#define WAIT_MMA(n_) \
    ...
    // ── Main loop, unrolled by 3 (NS=3) ───────────────────────────────────────
    // Slot pattern (k = 3*i + r):
    //   r=0: cur=0  nxt=2
    //   r=1: cur=1  nxt=0
    //   r=2: cur=2  nxt=1
    // With (num_tiles - 2) % 3 == 0, the outer loop has no remainder iters.
    static_assert(NS == 3, "u3 is hard-specialised to NUM_STAGES == 3");
    const int num_groups = (num_tiles - 2) / 3;  // = 42 at K=8192, BK=64
    for (int i = 0; i < num_groups; i++) {
        // r=0
        WAIT_SMEM(1);   __syncthreads();
        COMPUTE_TILE(0);
    ...
    // ── Drain ─────────────────────────────────────────────────────────────────
    // After main loop, the last cur was 2 (from the r=2 iter), and the next
    // unstarted compute is on slot 0 (next k % 3 = (num_tiles-2) % 3 = 0).
    #pragma unroll
...
void matmul_h2_s8_smem_wb_u3_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_##_ns##NS_(              \
    const __nv_bfloat16* __restrict__ A,                                         \
    const __nv_bfloat16* __restrict__ B,                                         \
    __nv_bfloat16* __restrict__ C,                                               \
    int M, int K, int N)                                                         \
{                                                                                \
    h2_s8_smem_wb_u3_impl<BM_, BN_, BK_, NG_, NS_>(A, B, C, M, K, N);                      \
}

// u3 specialised: only the single config we autotune over (BM=128 BN=256 BK=64 WG=2 NS=3).
