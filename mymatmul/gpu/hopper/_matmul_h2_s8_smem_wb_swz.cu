...
__device__ __forceinline__ void h2_s8_smem_wb_swz_impl(
    ...
    //
    // Pipeline loop (same structure will be reused for TMA backend in h2_s8_smem_wb_swz):
    //
...
void matmul_h2_s8_smem_wb_swz_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_##_ns##NS_##_gm##GM_(\
    const __nv_bfloat16* __restrict__ A,                                         \
    const __nv_bfloat16* __restrict__ B,                                         \
    __nv_bfloat16* __restrict__ C,                                               \
    int M, int K, int N)                                                         \
