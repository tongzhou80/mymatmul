...
/*
 * h2_s8_smem_wb_swz_pipe: h2_s8_smem_wb_flat with a single conservative
 * pre-barrier move. Currently holds: `_A_base = &A_sh[nxt][0][0]` moved
 * from inside P6 to BEFORE the wgmma.wait_group barrier.
 *
 * Used as a documented NULL RESULT — confirms (on top of swz, with the
 * broader autotune + median scoring) that moving any single compute
 * before either of the two waits is perf-neutral. The wait barriers
 * are not stalling in steady state, so there's no slot to hide compute
 * behind. Same conclusion was reached previously with `_pipe` (full
 * descriptor block before wait_smem), with PTX surgical relocation, and
 * with the more conservative `_bb` move on top of swz.
 *
 *   size 4096:  swz=699.2  pipe=693.1  (-0.9%, same config)
 *   size 6144:  swz=697.1  pipe=696.6  (0.0%,  same config)
 *   size 8192:  swz=733.1  pipe=717.3  (-2.2%, autotune picked NS=3 vs NS=4)
 *
 * Kept in the tree as documentation; not a perf variant.
 */
...
__device__ __forceinline__ void h2_s8_smem_wb_swz_pipe_impl(
    ...
    // Triton-style GROUP_M block reordering for better L2 reuse.
    int pid_m, pid_n;
    ...
    //
    // Pipeline loop (same structure will be reused for TMA backend in h2_s8_smem_wb_swz_pipe):
    //
...
#define WAIT_MMA(n_) \
    ...
    // ── Main loop (FULLY INLINED — phase comments mark logical sections) ─────
    //
    //   Per K-iter pipeline shape (single commit per LOAD_TILE, NS-stage cp.async):
    //     P1  WAIT  cp.async   — keep NS-2 cmts in flight (tile `cur` lands now)
    //     P2  SYNC  CTA-wide   — all warpgroups see the just-arrived tile data
    //     P3  COMPUTE wgmma    — descriptor build + kk loop + commit
    //     P4  WAIT  wgmma(1)   — keep 1 wgmma group in flight (slot `nxt` freed)
    //     P5  SYNC  CTA-wide   — bridge per-warpgroup wgmma drain to all threads
    //     P6  ISSUE cp.async   — A then B into slot `nxt`, one commit_group
    //
    for (int k = 0; k < num_tiles - (NS - 1); k++) {
        const int cur = k % NS;
        const int nxt = (k + NS - 1) % NS;

        // ── P1: WAIT cp.async — keep NS-2 commit groups in flight ─────────────
        __pipeline_wait_prior(NS - 2);

        // ── P2: CTA-wide barrier — bridge per-thread cp.async to all 256 threads
        __syncthreads();

        // ── P3: COMPUTE — fence + (BK/16) × M_ITERS wgmmas + commit ───────────
        wgmma_fence();
        #pragma unroll
        for (int _kk = 0; _kk < BK / 16; _kk++) {
            // B descriptor: SMEM addr of slot `cur` advanced by kk × 2048 B
            const uint32_t _bb = (uint32_t)__cvta_generic_to_shared(
                                     &B_sh[cur][0][0][0]);
            const uint64_t _db = make_wgmma_b_desc<BN, BK>(
                                     (uint32_t)(_bb + _kk * K_STEP_BYTES));
        ...
        // ── P3b: PRECOMPUTE A SMEM base for next tile (slot `nxt`) ────────────
        //   Lifted out of P6 — pure arithmetic on `nxt`, hidden behind wait_mma.
        char* const _A_base = (char*)&A_sh[nxt][0][0];

        // ── P4: WAIT wgmma — keep 1 group in flight, free slot `nxt` ──────────
        asm volatile("wgmma.wait_group.sync.aligned 1;\n" ::: "memory");

        // ── P5: CTA-wide barrier — bridge wgmma drain to all warpgroups ───────
        __syncthreads();

        // ── P6: ISSUE cp.async for tile `nxt` (A loads then B loads, 1 commit)
        {
            #pragma unroll
            for (int _i = 0; _i < A_GROUPS; _i++) {
                __pipeline_memcpy_async(
            ...
            char* const _B_base = (char*)&B_sh[nxt][0][0][0];
            #pragma unroll
            for (int _i = 0; _i < B_GROUPS; _i++) {
                __pipeline_memcpy_async(
                    _B_base + B_sh_off[_i],
...
#define MAKE_LAUNCHER(BM_, BN_, BK_, NG_, NS_, GM_)                              \
extern "C" __global__ __launch_bounds__(NG_ * 128, LB_MIN_BLOCKS)               \
void matmul_h2_s8_smem_wb_swz_pipe_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_##_ns##NS_##_gm##GM_(\
    const __nv_bfloat16* __restrict__ A,                                         \
    const __nv_bfloat16* __restrict__ B,                                         \
    __nv_bfloat16* __restrict__ C,                                               \
    int M, int K, int N)                                                         \
{                                                                                \
    h2_s8_smem_wb_swz_pipe_impl<BM_, BN_, BK_, NG_, NS_, GM_>(A, B, C, M, K, N);       \
}
