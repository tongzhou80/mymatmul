...
/*
 * H2 Stage 8: TMA + wgmma RS + wgmma.wait_group 1.
 *
 * Single change from h2_s5: wgmma.wait_group 0 → wait_group 1.
 *
 * In h2_s5, COMPUTE_TILE ended with wgmma.wait_group 0 — fully draining the
 * tensor core before proceeding. The tensor core sat idle while the CPU reset
 * the next doorbell and kicked off the next TMA load.
 *
 * Here, after committing wgmma group k we only wait for group k-1. Group k
 * stays in the tensor core pipeline while thread 0 resets the doorbell and
 * issues the next TMA load. Tensor core and TMA engine run concurrently.
 *
 * Loop structure (vs h2_s5):
 *   h2_s5: [LOAD_TILE] → doorbell_wait → MULTIPLY(wait_group 0) → sync
 *   h2_s8: doorbell_wait → sync → COMPUTE(no drain) → wait_group(1) → [LOAD_TILE]
 *
 * LOAD_TILE moves to AFTER wait_group(1): wait_group(1) at iter k proves
 * wgmma[k-1] has released SMEM slot (k-1)%NS, making it safe for TMA to write.
 */
...
__device__ __forceinline__ void h2s8_impl(
    const TmaDesc& tma_A,
    const TmaDesc& tma_B,
...
#define LOAD_TILE(slot_, k_start_)                                                  \
    ...
    // ── Prologue: prefill NS-1 pipeline slots ────────────────────────────────
    // Reset just the NS-1 slots we're loading. The remaining slot (NS-1) will
    // be reset just-in-time at the end of iteration k=0.
    if (tid == 0) {
        #pragma unroll
        for (int s = 0; s < NS - 1; s++) {
            doorbell_reset(&mbar[s]);
            LOAD_TILE(s, s * BK);
        }
    }
    __syncthreads();

    // ── COMPUTE_TILE: wgmma compute — commit but DO NOT drain ─────────────────
    // Unlike h2_s5's MULTIPLY_TILE, we omit wgmma_drain() (wait_group 0).
    // Caller calls wgmma_wait_1() after, keeping group k in flight while
    // thread 0 resets the doorbell and issues the next TMA tile.
#define COMPUTE_TILE(slot_)                                                         \
    ...
    // ── Main K loop ───────────────────────────────────────────────────────────
    // Structure (vs h2_s5):
    //   h2_s5: LOAD_TILE(nxt) → doorbell_wait(cur) → MULTIPLY (drain0) → sync
    //   h2_s8: doorbell_wait(cur) → sync → COMPUTE (no drain) → wait_group(1)
    //          → LOAD_TILE(nxt)    [LOAD after wait_1: slot nxt is provably free]
    for (int k = 0; k < num_tiles - (NS - 1); k++) {
        const int cur = k % NS;
        const int nxt = (k + NS - 1) % NS;

        doorbell_wait(&mbar[cur]);    // cur tile is in SMEM
        __syncthreads();

        COMPUTE_TILE(cur);            // acc += A[cur] × B[cur], group k committed
        wgmma_wait_1();               // groups 0..k-1 done; group k still in flight

        // Issue next tile AFTER wait_group(1): slot nxt = (k-1)%NS was read by
        // wgmma[k-1] which is now confirmed done — safe to overwrite with TMA.
        if (tid == 0) {
    ...
    wgmma_drain();    // wait_group(0): drain the final group before epilogue
