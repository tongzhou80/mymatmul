...
/*
 * h2_s8: h2_s7_runptr + flattened SMEM destination offsets.
 *
...
__device__ __forceinline__ void h2_s8_impl(
...
void matmul_h2_s8_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_##_ns##NS_(              \
    const __nv_bfloat16* __restrict__ A,                                         \
    const __nv_bfloat16* __restrict__ B,                                         \
    __nv_bfloat16* __restrict__ C,                                               \
    int M, int K, int N)                                                         \
{                                                                                \
    h2_s8_impl<BM_, BN_, BK_, NG_, NS_>(A, B, C, M, K, N);                      \
}
