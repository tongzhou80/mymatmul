...
__device__ __forceinline__ void h2s7_impl(
    ...
    int M, int K, int N,
    uint64_t* __restrict__ timings   // [4 × num_CTAs] cycle counts: wait_smem, compute, wait_mma, load
) {
...
#define WAIT_MMA(n_) \
    ...
    // ── Main loop (instrumented) ─────────────────────────────────────────────
    //   Phase breakdown (6 phases per iter):
    //     0 WAIT_SMEM       — cp.async drain  (just __pipeline_wait_prior)
    //     1 sync_pre_cmp    — __syncthreads before COMPUTE_TILE
    //     2 COMPUTE_TILE    — wgmma.fence + 4×wgmma + commit
    //     3 WAIT_MMA(1)     — wait for prev wgmma group
    //     4 sync_pre_load   — __syncthreads before LOAD_TILE
    //     5 LOAD_TILE       — 12×cp.async + commit
    uint64_t t_phase[6] = {0,0,0,0,0,0};
    ...
    // Thread 0 of each CTA writes its accumulated timings (in cycles)
    if (tid == 0) {
        const int cta_id = blockIdx.y * gridDim.x + blockIdx.x;
        #pragma unroll
        for (int i = 0; i < 6; i++) timings[cta_id * 6 + i] = t_phase[i];
    }
...
void matmul_h2s7_timed_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_##_ns##NS_(        \
    const __nv_bfloat16* __restrict__ A,                                         \
    const __nv_bfloat16* __restrict__ B,                                         \
    __nv_bfloat16* __restrict__ C,                                               \
    ...
    int M, int K, int N)                                                         \
{                                                                                \
    h2s7_impl<BM_, BN_, BK_, NG_, NS_>(A, B, C, M, K, N, timings);             \
}
