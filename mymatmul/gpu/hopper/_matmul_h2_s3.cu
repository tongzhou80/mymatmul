 ...
 *
 * Key change vs H2-S2:
 *   BM / threads  : fixed 64 / 128 → NUM_WG*64 / NUM_WG*128
 *
 * With 2 warpgroups (BM=128, 256 threads):
 *   - Both warpgroups share the same B tile (loaded once per pipeline stage).
 *   - Each warpgroup independently loads its 64-row A slice and runs wgmma.
 *   - wgmma.fence / commit_group / wait_group are per-warpgroup; __syncthreads
 *     synchronises the full CTA between pipeline stages.
 *
 * wgmma m64nBNk16 with transB=1 (N-major B, i.e. B stored [BK][BN] row-major):
 *   A : 4 uint32 registers per thread (ldmatrix_x4, each warpgroup loads its 64 rows)
 *   B : uint64 SMEM descriptor — shared by all warpgroups
...
template<int NUM_WG, int BN, int BK>
__device__ __forceinline__ void h2s3_impl(
    const TmaDesc& tma_A,
    const TmaDesc& tma_B,
    __nv_bfloat16* __restrict__ C,
    int M, int K, int N
) {
    constexpr int BM = NUM_WG * 64;  // each warpgroup covers 64 rows
    ...
    for (int k = 0; k < num_tiles - 1; k++) {
        ...
        for (int kk = 0; kk < BK / 16; kk++) {
            uint32_t a[4];
            const int a_row     = wg_id * 64 + local_warp * 16 + (lane % 16);
...
#define MAKE_LAUNCHER(NG_, BN_, BK_)                                             \
extern "C" __global__ __launch_bounds__(NG_ * 128, LB_MIN_BLOCKS)               \
void matmul_h2s3_wg##NG_##_bn##BN_##_bk##BK_(                                   \
    const __grid_constant__ TmaDesc tma_A,                                       \
    const __grid_constant__ TmaDesc tma_B,                                       \
    __nv_bfloat16* __restrict__ C, int M, int K, int N)                          \
{                                                                                 \
    h2s3_impl<NG_, BN_, BK_>(tma_A, tma_B, C, M, K, N);                         \
}

// 1 warpgroup (BM=64)
MAKE_LAUNCHER(1,  64, 16) MAKE_LAUNCHER(1,  64, 32) MAKE_LAUNCHER(1,  64, 64)
MAKE_LAUNCHER(1, 128, 16) MAKE_LAUNCHER(1, 128, 32) MAKE_LAUNCHER(1, 128, 64)
MAKE_LAUNCHER(1, 256, 16) MAKE_LAUNCHER(1, 256, 32) MAKE_LAUNCHER(1, 256, 64)

// 2 warpgroups (BM=128); both share B tile, each owns 64 rows of A/C
MAKE_LAUNCHER(2,  64, 16) MAKE_LAUNCHER(2,  64, 32) MAKE_LAUNCHER(2,  64, 64)
MAKE_LAUNCHER(2, 128, 16) MAKE_LAUNCHER(2, 128, 32) MAKE_LAUNCHER(2, 128, 64)
MAKE_LAUNCHER(2, 256, 16) MAKE_LAUNCHER(2, 256, 32) MAKE_LAUNCHER(2, 256, 64)
