...
/*
 * h2_s9: h2_s8 + ring counters for cur/nxt slot (avoid k % NS magic-mod).
 *
 * Motivation (PTX inspection of s8): `cur = k % NS` and `nxt = (k+NS-1) % NS`
 * each compile to a 5-op magic-number sequence (mul.wide × magic, shr, cvt,
 * mul × NS, sub) → ~10 ops/iter. Replacing with two ring counters advanced
 * via branchless wrap should compile to ~4 ops/iter.
 */
...
__device__ __forceinline__ void h2_s9_impl(
    ...
    //
    // Pipeline loop (same structure will be reused for TMA backend in h2_s9):
    //
...
void matmul_h2_s9_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_##_ns##NS_(              \
    const __nv_bfloat16* __restrict__ A,                                         \
    const __nv_bfloat16* __restrict__ B,                                         \
    __nv_bfloat16* __restrict__ C,                                               \
    int M, int K, int N)                                                         \
{                                                                                \
    h2_s9_impl<BM_, BN_, BK_, NG_, NS_>(A, B, C, M, K, N);                      \
}
