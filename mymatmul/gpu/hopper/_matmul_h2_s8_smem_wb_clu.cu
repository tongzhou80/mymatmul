...
/*
 * h2_s8_smem_wb_clu: h2_s8_smem_wb + __cluster_dims__(2, 1, 1) on the launcher.
 *
 * Source code is byte-identical to h2_s8_smem_wb; the only difference is
 * that the launcher is annotated with __cluster_dims__(2, 1, 1). Hopper
 * guarantees CTAs in a cluster co-reside on the same GPC, which improves
 * L2 hit rate on the A row band shared by the 2 adjacent N-tiles. No
 * DSMEM / cluster.sync features are used; we're only paying for the
 * co-location heuristic.
 *
 * Specialised to the known-best autotune config (BM=128 BN=256 BK=64 WG=2
 * NS=3). Requires gridDim.x divisible by 2 → N must be a multiple of 2*BN
 * (= 512). All standard square sizes ≥ 512 satisfy this.
 */
...
__device__ __forceinline__ void h2_s8_smem_wb_clu_impl(
    ...
    //
    // Pipeline loop (same structure will be reused for TMA backend in h2_s8_smem_wb_clu):
    //
...
void matmul_h2_s8_smem_wb_clu_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_##_ns##NS_##_cx##CX_##_cy##CY_(\
    const __nv_bfloat16* __restrict__ A,                                         \
    const __nv_bfloat16* __restrict__ B,                                         \
    __nv_bfloat16* __restrict__ C,                                               \
    int M, int K, int N)                                                         \
{                                                                                \
    h2_s8_smem_wb_clu_impl<BM_, BN_, BK_, NG_, NS_>(A, B, C, M, K, N);            \
}

// Specialised: known-best base config + single cluster shape (2,1,1).
MAKE_LAUNCHER(128, 256, 64, 2, 3, 2, 1)
