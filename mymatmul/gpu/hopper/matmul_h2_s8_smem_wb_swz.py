...
_CU_PATH    = os.path.join(_HOPPER_DIR, "_matmul_h2_s8_smem_wb_swz.cu")
_NVCC       = "/usr/local/cuda/bin/nvcc"
_ARCH       = "sm_90a"
_CUBIN      = os.path.join(_HOPPER_DIR, f"_matmul_h2_s8_smem_wb_swz_{_ARCH}.cubin")
...
def _get_mod():
    ...
    with _mod_lock:
        ...
        if not os.path.exists(_CUBIN) or os.path.getmtime(_CU_PATH) > os.path.getmtime(_CUBIN):
            print(f"[h2_s8_smem_wb_swz] compiling for {_ARCH} ...", end=" ", flush=True)
...
# swz is specialised: only the known-best base config; autotune over GROUP_M.
_BMS = [128]
_BNS = [256]
_BKS = [64]
_NWS = [2]
...
def _kname(bm, bn, bk, nwg, ns, gm):
    return f"matmul_h2_s8_smem_wb_swz_bm{bm}_bn{bn}_bk{bk}_wg{nwg}_ns{ns}_gm{gm}"
...
def _tune(M, N, K):
    ...
    for i, (bm, bn, bk, nwg, ns, gm) in enumerate(cfgs):
        ...
        if ms < best_t:
            best_t, best = ms, (bm, bn, bk, nwg, ns, gm)
    return best


def matmul_h2_s8_smem_wb_swz(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    M, K = A.shape;  _, N = B.shape
    key = (M, N, K)
    if key not in _best:
        cfgs = [c for c in _CONFIGS
        ...
        print(f"[h2_s8_smem_wb_swz] autotuning {M}×{N}×{K} over {len(cfgs)} configs ...")
        _best[key] = _tune(M, N, K)
        bm, bn, bk, nwg, ns, gm = _best[key]
        mi = _m_iters(bm, nwg)
        print(f"[h2_s8_smem_wb_swz] best: BM={bm} BN={bn} BK={bk} WG={nwg} NS={ns} GM={gm} M_ITERS={mi}")
    bm, bn, bk, nwg, ns, gm = _best[key]
