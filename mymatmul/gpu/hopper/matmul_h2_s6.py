...
Key changes vs h2_s5:
  - cp.async instead of TMA: regular pointers, __pipeline_wait_prior, no mbarrier.
  - wgmma SS mode: A read via GmmaDescriptor (no ldmatrix), extra transA=0 param.
  - A swizzle period = BK/8 for all BK (BK=16/32/64 → B32/B64/B128).
  - B stored as sub-tile format in SMEM: (BN/64) sub-tiles of [BK][64] per stage.
  - Smaller SMEM: BK=32 now viable with correct swizzle → 96 KB for BM=128,BN=256.
"""
...
_CU_PATH    = os.path.join(_HOPPER_DIR, "_matmul_h2_s6.cu")
_NVCC       = "/usr/local/cuda/bin/nvcc"
_ARCH       = "sm_90a"
_CUBIN      = os.path.join(_HOPPER_DIR, f"_matmul_h2s6_{_ARCH}.cubin")
...
def _get_mod():
    ...
    with _mod_lock:
        ...
        if not os.path.exists(_CUBIN) or os.path.getmtime(_CU_PATH) > os.path.getmtime(_CUBIN):
            print(f"[h2s6] compiling for {_ARCH} ...", end=" ", flush=True)
...
def _kname(bm, bn, bk, nwg, ns):
    return f"matmul_h2s6_bm{bm}_bn{bn}_bk{bk}_wg{nwg}_ns{ns}"
...
def matmul_h2_s6(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    M, K = A.shape;  _, N = B.shape
    key = (M, N, K)
    if key not in _best:
        cfgs = [(bm, bn, bk, nwg, ns) for bm, bn, bk, nwg, ns in _CONFIGS
                if M % bm == 0 and N % bn == 0 and K % bk == 0
                and K // bk >= ns]
        print(f"[h2_s6] autotuning {M}×{N}×{K} over {len(cfgs)} configs ...")
        _best[key] = _tune(M, N, K)
        bm, bn, bk, nwg, ns = _best[key]
        mi = _m_iters(bm, nwg)
        print(f"[h2_s6] best: BM={bm} BN={bn} BK={bk} WG={nwg} NS={ns} M_ITERS={mi}")
    bm, bn, bk, nwg, ns = _best[key]
