"""H5: h2_s7 with descriptor-advance optimization (no descriptor rebuild per wgmma).

Identical to h2_s7 except COMPUTE_TILE precomputes A/B descriptors at kk=0 and
advances them via `+=` per kk step, mirroring what Triton's autotuned PTX does.
Cuts ~6 integer instructions per wgmma issue, raising the warp's wgmma issue rate.
"""
