...
_CU_PATH    = os.path.join(_HOPPER_DIR, "_matmul_h2_s8_smem_wb_pipe.cu")
_NVCC       = "/usr/local/cuda/bin/nvcc"
_ARCH       = "sm_90a"
_CUBIN      = os.path.join(_HOPPER_DIR, f"_matmul_h2_s8_smem_wb_pipe_{_ARCH}.cubin")
...
def _get_mod():
    ...
    with _mod_lock:
        ...
        if not os.path.exists(_CUBIN) or os.path.getmtime(_CU_PATH) > os.path.getmtime(_CUBIN):
            print(f"[h2_s8_smem_wb_pipe] compiling for {_ARCH} ...", end=" ", flush=True)
...
def _kname(bm, bn, bk, nwg, ns):
    return f"matmul_h2_s8_smem_wb_pipe_bm{bm}_bn{bn}_bk{bk}_wg{nwg}_ns{ns}"
...
def matmul_h2_s8_smem_wb_pipe(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    M, K = A.shape;  _, N = B.shape
    key = (M, N, K)
    if key not in _best:
        cfgs = [(bm, bn, bk, nwg, ns) for bm, bn, bk, nwg, ns in _CONFIGS
                if M % bm == 0 and N % bn == 0 and K % bk == 0
                and K // bk >= ns]
        print(f"[h2_s8_smem_wb_pipe] autotuning {M}×{N}×{K} over {len(cfgs)} configs ...")
        _best[key] = _tune(M, N, K)
        bm, bn, bk, nwg, ns = _best[key]
        mi = _m_iters(bm, nwg)
        print(f"[h2_s8_smem_wb_pipe] best: BM={bm} BN={bn} BK={bk} WG={nwg} NS={ns} M_ITERS={mi}")
    bm, bn, bk, nwg, ns = _best[key]
