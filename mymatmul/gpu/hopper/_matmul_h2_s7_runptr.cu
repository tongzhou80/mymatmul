...
/*
 * h2_s7_runptr: h2_s7_split + Triton's per-thread running-pointer pattern.
 *
 * Motivation (from t36 decompilation comparison vs Triton):
 *   s7_split's LOAD_TILE recomputes the global address from base each cp.async
 *   ( `&A[(block_row + _r) * K + (k0_) + _c]` ) — about 3 ops per cp.async.
 *   With 6 cp.async per iter that's ≈18 integer ops/iter for address compute.
 *
 *   Triton maintains per-thread running pointers (A_curr[i], B_curr[i]) that
 *   advance by `BK` (A) or `BK*N` (B) elements per K-iter — about 3 add.s64
 *   total per iter. On a warp-issue-bound kernel (s7_cycle_breakdown.md),
 *   the saved ≈15 ops/iter could close the gap to Triton.
 */
...
__device__ __forceinline__ void h2_s7_runptr_impl(
...
void matmul_h2_s7_runptr_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_##_ns##NS_(              \
    const __nv_bfloat16* __restrict__ A,                                         \
    const __nv_bfloat16* __restrict__ B,                                         \
    __nv_bfloat16* __restrict__ C,                                               \
    int M, int K, int N)                                                         \
{                                                                                \
    h2_s7_runptr_impl<BM_, BN_, BK_, NG_, NS_>(A, B, C, M, K, N);                      \
}
