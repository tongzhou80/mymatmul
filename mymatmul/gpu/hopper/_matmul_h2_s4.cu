...
template<int BM, int BN, int BK, int NUM_WG>
__device__ __forceinline__ void h2s4_impl(
    const TmaDesc& tma_A,
    const TmaDesc& tma_B,
...
// BM is now explicit in the name and template.
#define MAKE_LAUNCHER(BM_, BN_, BK_, NG_)                                        \
extern "C" __global__ __launch_bounds__(NG_ * 128, LB_MIN_BLOCKS)               \
void matmul_h2s4_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_(                         \
    const __grid_constant__ TmaDesc tma_A,                                       \
    const __grid_constant__ TmaDesc tma_B,                                       \
    __nv_bfloat16* __restrict__ C, int M, int K, int N)                          \
{                                                                                 \
    h2s4_impl<BM_, BN_, BK_, NG_>(tma_A, tma_B, C, M, K, N);                   \
}

// M_ITERS = BM / (NUM_WG * 64).  Exclude BM=256,NG=1,BN=256 (acc=512 regs > budget).
// NW=1 warpgroup
MAKE_LAUNCHER( 64,  64, 16, 1) MAKE_LAUNCHER( 64,  64, 32, 1) MAKE_LAUNCHER( 64,  64, 64, 1)
MAKE_LAUNCHER( 64, 128, 16, 1) MAKE_LAUNCHER( 64, 128, 32, 1) MAKE_LAUNCHER( 64, 128, 64, 1)
MAKE_LAUNCHER( 64, 256, 16, 1) MAKE_LAUNCHER( 64, 256, 32, 1) MAKE_LAUNCHER( 64, 256, 64, 1)
// BM=128, NG=1: M_ITERS=2
MAKE_LAUNCHER(128,  64, 16, 1) MAKE_LAUNCHER(128,  64, 32, 1) MAKE_LAUNCHER(128,  64, 64, 1)
MAKE_LAUNCHER(128, 128, 16, 1) MAKE_LAUNCHER(128, 128, 32, 1) MAKE_LAUNCHER(128, 128, 64, 1)
MAKE_LAUNCHER(128, 256, 16, 1) MAKE_LAUNCHER(128, 256, 32, 1) MAKE_LAUNCHER(128, 256, 64, 1)
// BM=256, NG=1: M_ITERS=4, BN=256 excluded (4*128=512 regs = limit)
MAKE_LAUNCHER(256,  64, 16, 1) MAKE_LAUNCHER(256,  64, 32, 1) MAKE_LAUNCHER(256,  64, 64, 1)
MAKE_LAUNCHER(256, 128, 16, 1) MAKE_LAUNCHER(256, 128, 32, 1) MAKE_LAUNCHER(256, 128, 64, 1)

// NW=2 warpgroups
MAKE_LAUNCHER(128,  64, 16, 2) MAKE_LAUNCHER(128,  64, 32, 2) MAKE_LAUNCHER(128,  64, 64, 2)
MAKE_LAUNCHER(128, 128, 16, 2) MAKE_LAUNCHER(128, 128, 32, 2) MAKE_LAUNCHER(128, 128, 64, 2)
MAKE_LAUNCHER(128, 256, 16, 2) MAKE_LAUNCHER(128, 256, 32, 2) MAKE_LAUNCHER(128, 256, 64, 2)
// BM=256, NG=2: M_ITERS=2
MAKE_LAUNCHER(256,  64, 16, 2) MAKE_LAUNCHER(256,  64, 32, 2) MAKE_LAUNCHER(256,  64, 64, 2)
MAKE_LAUNCHER(256, 128, 16, 2) MAKE_LAUNCHER(256, 128, 32, 2) MAKE_LAUNCHER(256, 128, 64, 2)
MAKE_LAUNCHER(256, 256, 16, 2) MAKE_LAUNCHER(256, 256, 32, 2) MAKE_LAUNCHER(256, 256, 64, 2)
