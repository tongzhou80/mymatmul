...
/*
 * h2_s8_smem_wb_flat: same kernel as h2_s8_smem_wb_swz with the K-loop
 * macros inlined so the whole main loop body sits in one block of code.
 *
 * Purpose is readability, not perf — having every instruction visible in
 * sequence makes it easier to spot redundant ops, misplaced barriers, or
 * scheduling opportunities than chasing the LOAD_TILE / COMPUTE_TILE
 * macros around the file. The prologue and drain still use macros since
 * they're not the hot path.
 *
 * Functionally identical to h2_s8_smem_wb_swz (autotunes BM/BN/BK/NW/NS
 * and GROUP_M ∈ {1,2,4,8}).
 */
...
__device__ __forceinline__ void h2_s8_smem_wb_flat_impl(
    ...
    //
    // Pipeline loop (same structure will be reused for TMA backend in h2_s8_smem_wb_flat):
    //
...
void matmul_h2_s8_smem_wb_flat_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_##_ns##NS_##_gm##GM_(\
    const __nv_bfloat16* __restrict__ A,                                         \
    const __nv_bfloat16* __restrict__ B,                                         \
    __nv_bfloat16* __restrict__ C,                                               \
    int M, int K, int N)                                                         \
{                                                                                \
    h2_s8_smem_wb_flat_impl<BM_, BN_, BK_, NG_, NS_, GM_>(A, B, C, M, K, N);       \
}
