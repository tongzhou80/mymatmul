...
_CU_PATH    = os.path.join(_HOPPER_DIR, "_matmul_h4_shapes.cu")
_NVCC       = "/usr/local/cuda/bin/nvcc"
_ARCH       = "sm_90a"
_CUBIN      = os.path.join(_HOPPER_DIR, f"_matmul_h4_shapes_{_ARCH}.cubin")
...
def _get_mod():
    ...
    with _mod_lock:
        ...
        if not os.path.exists(_CUBIN) or os.path.getmtime(_CU_PATH) > os.path.getmtime(_CUBIN):
            print(f"[h4_shapes] compiling for {_ARCH} ...", end=" ", flush=True)
...
# ── Config space ──────────────────────────────────────────────────────────────
# Must mirror the launchers generated in _matmul_h4_shapes.cu exactly.

# Cluster shapes (CX, CY) generated in the .cu
_CLUSTERS = [(1, 1), (1, 2), (2, 1), (2, 2), (1, 4), (4, 1), (2, 4), (4, 2)]

# (BM, BN, BK, WG) combos generated in _matmul_h4_shapes.cu — keep in sync.
_BMBN_BK_WG = [
...
def _kname(bm, bn, bk, nwg, ns, cx, cy):
    return f"matmul_h4_shapes_bm{bm}_bn{bn}_bk{bk}_wg{nwg}_ns{ns}_cx{cx}_cy{cy}"
...
def matmul_h4_shapes(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    M, K = A.shape;  _, N = B.shape
    key = (M, N, K)
    if key not in _best:
        cfgs = [c for c in _CONFIGS
                ...
                and K % c[2] == 0
                and K // c[2] >= c[4]]
        print(f"[h4_shapes] autotuning {M}×{N}×{K} over {len(cfgs)} configs ...")
        _best[key] = _tune(M, N, K)
        bm, bn, bk, nwg, ns, cx, cy = _best[key]
        mi = _m_iters(bm, nwg)
        print(f"[h4_shapes] best: BM={bm} BN={bn} BK={bk} WG={nwg} NS={ns} "
