...
/*
 * H1 multi-stage: tc5_lb with a tunable NUM_STAGES pipeline depth.
 *
...
__device__ __forceinline__ void h1ms_impl(
...
void matmul_h1ms_bm##BM_##_bn##BN_##_bk##BK_##_nw##NW_##_ns##NS_(                 \
    const __nv_bfloat16* __restrict__ A, const __nv_bfloat16* __restrict__ B,      \
    __nv_bfloat16* __restrict__ C, int M, int K, int N)                            \
{                                                                                   \
    h1ms_impl<BM_, BN_, BK_, NW_, NS_>(A, B, C, M, K, N);                         \
}
