...
/*
 * h2_s8_smem_wb_pipe: h2_s8_smem_wb with split MMA / LOAD phases for overlap.
 *
 * The main loop is restructured so the post-barrier PTX contains only the
 * actual async-unit dispatch (wgmma / cp.async), with all auxiliary compute
 * moved into precompute slots that overlap the WAIT_SMEM and WAIT_MMA stalls:
 *
 *   MMA_PRECOMPUTE(cur);    // build all wgmma A/B descriptors (regs only)
 *   BARRIER_DATA_READY();   // WAIT_SMEM + __syncthreads
 *   MMA_ASYNC();            // fence + wgmma.mma_async × (BK/16) × M_ITERS + commit
 *
 *   LOAD_TILE_PRECOMPUTE(nxt);  // compute per-cp.async SMEM dest base
 *   WAIT_MMA(1);
 *   __syncthreads();
 *   LOAD_TILE_ASYNC();          // fire cp.async × (A_GROUPS + B_GROUPS) + commit
 *
 * Descriptor compute (8 desc * ~6 ops each ≈ 50 ops/iter) is hidden behind
 * the WAIT_SMEM stall, ideally letting wgmma fire immediately when data arrives.
 */
...
__device__ __forceinline__ void h2_s8_smem_wb_pipe_impl(
    ...
    //
    // Pipeline loop (same structure will be reused for TMA backend in h2_s8_smem_wb_pipe):
    //
    ...
    // ── Split-phase pipeline macros ───────────────────────────────────────────
    //
    // LOAD_TILE_PRECOMPUTE / LOAD_TILE_ASYNC, MMA_PRECOMPUTE / MMA_ASYNC.
    // PRECOMPUTE phases run while a wait is in flight; ASYNC phases contain
    // only the actual async-unit dispatch (cp.async, wgmma).

    // PRECOMPUTE: stash the per-tile SMEM dest base ptrs in regs. Inputs:
    // slot_. Outputs: _A_base_nxt, _B_base_nxt (host-side variables declared
    // by the user before invoking the macro).
#define LOAD_TILE_PRECOMPUTE(slot_)                                                 \
    do {                                                                            \
        _A_base_nxt = (char*)&A_sh[(slot_)][0][0];                                  \
        _B_base_nxt = (char*)&B_sh[(slot_)][0][0][0];                               \
    } while (0)

    // ASYNC: fire cp.async × (A_GROUPS + B_GROUPS) using precomputed bases +
    // per-(tid,_i) offsets, then a single commit. A_curr / B_curr advance.
#define LOAD_TILE_ASYNC()                                                           \
    do {                                                                            \
        _Pragma("unroll")                                                           \
        for (int _i = 0; _i < A_GROUPS; _i++) {                                    \
            __pipeline_memcpy_async(                                                \
                _A_base_nxt + A_sh_off[_i],                                         \
                A_curr[_i],                                                         \
    ...
    // Combined macro for prologue / drain (use the split phases in the main loop)
#define LOAD_TILE(slot_)                                                            \
    do {                                                                            \
        LOAD_TILE_PRECOMPUTE(slot_);                                                \
        LOAD_TILE_ASYNC();                                                          \
    } while (0)

#define WAIT_SMEM(n_) __pipeline_wait_prior(n_)

    // MMA_PRECOMPUTE: build all wgmma A/B descriptors into register arrays
    // _da[BK/16][M_ITERS] / _db[BK/16] (declared by host). Only depends on
    // slot_ and SMEM base addresses — no SMEM data access — so it can run
    // entirely before WAIT_SMEM lands.
#define MMA_PRECOMPUTE(slot_)                                                       \
    do {                                                                            \
        _Pragma("unroll")                                                           \
        for (int _kk = 0; _kk < BK / 16; _kk++) {                                  \
            const uint32_t _bb = (uint32_t)__cvta_generic_to_shared(               \
                                     &B_sh[(slot_)][0][0][0]);                     \
            _db[_kk] = make_wgmma_b_desc<BN, BK>(                                   \
            ...
            for (int _m = 0; _m < M_ITERS; _m++) {                                 \
                ...
                const uint32_t _aa = (uint32_t)__cvta_generic_to_shared(           \
                                         &A_sh[(slot_)][_mrow][0]);                \
                _da[_kk][_m] = make_wgmma_a_desc<BK>(_aa, _kk);                    \
            }                                                                       \
        }                                                                           \
    } while (0)

    // MMA_ASYNC: fence + wgmma fires + commit. Reads precomputed _da/_db only.
#define MMA_ASYNC()                                                                 \
    do {                                                                            \
        wgmma_fence();                                                              \
        _Pragma("unroll")                                                           \
        for (int _kk = 0; _kk < BK / 16; _kk++) {                                  \
            _Pragma("unroll")                                                       \
            for (int _m = 0; _m < M_ITERS; _m++) {                                 \
                wgmma_ss_call<BN>((float*)acc[_m], _da[_kk][_m], _db[_kk]);        \
            }                                                                       \
        }                                                                           \
        wgmma_commit();                                                             \
    } while (0)

    // Combined macro for the drain (no overlap opportunity there)
#define COMPUTE_TILE(slot_)                                                         \
    do {                                                                            \
        MMA_PRECOMPUTE(slot_);                                                      \
        MMA_ASYNC();                                                                \
    } while (0)
...
#define WAIT_MMA(n_) \
    ...
    constexpr int K_ITERS = BK / 16;

    // ── Prologue ──────────────────────────────────────────────────────────────
    #pragma unroll
    for (int s = 0; s < NS - 1; s++) {
        // declared here so prologue's LOAD_TILE compiles (macros write to them)
        char* _A_base_nxt;
        char* _B_base_nxt;
        LOAD_TILE(s);
    }
    ...
    for (int k = 0; k < num_tiles - (NS - 1); k++) {
        ...
        // wgmma descriptors live in regs only from PRECOMPUTE → ASYNC; loop-scoped.
        uint64_t _da[K_ITERS][M_ITERS];
        uint64_t _db[K_ITERS];
        MMA_PRECOMPUTE(cur);
        WAIT_SMEM(NS - 2);
        __syncthreads();
        MMA_ASYNC();

        // Same for the next-tile SMEM dest bases.
        char* _A_base_nxt;
        char* _B_base_nxt;
        LOAD_TILE_PRECOMPUTE(nxt);
        WAIT_MMA(1);
        __syncthreads();
        LOAD_TILE_ASYNC();
    }
...
#undef LOAD_TILE
#undef LOAD_TILE_PRECOMPUTE
#undef LOAD_TILE_ASYNC
#undef WAIT_SMEM
#undef COMPUTE_TILE
#undef MMA_PRECOMPUTE
#undef MMA_ASYNC
#undef WAIT_MMA
...
void matmul_h2_s8_smem_wb_pipe_bm##BM_##_bn##BN_##_bk##BK_##_wg##NG_##_ns##NS_(              \
    const __nv_bfloat16* __restrict__ A,                                         \
    const __nv_bfloat16* __restrict__ B,                                         \
    __nv_bfloat16* __restrict__ C,                                               \
    int M, int K, int N)                                                         \
{                                                                                \
    h2_s8_smem_wb_pipe_impl<BM_, BN_, BK_, NG_, NS_>(A, B, C, M, K, N);                      \
}
