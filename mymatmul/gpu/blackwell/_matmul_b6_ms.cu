// b6_ms: b5_tma + multi-stage TMA prefetch pipeline.
//
// Adds NUM_STAGES-deep SMEM ring buffer on top of b5_tma.  Main loop follows
// the same prologue/main/drain shape as hopper/_matmul_h1_ms.cu:
//
//     for (k = 0..num_tiles - (NS - 1) - 1) {
//         LOAD_TILE(nxt, ...);          // issue next load (async)
//         WAIT_ON_TILE_READY(cur);      // wait for current tile (NS-1 in flight)
//         MMA_ASYNC(cur, ...);          // issue all K_MMAS for current tile
//         WAIT_ON_MMA(cur);             // 1 MMA in flight at a time
//     }
//
// The load/compute overlap is the source of the speedup: the TMA load runs
// concurrently with the MMA below it in the loop body, hiding DRAM→SMEM latency.
//
...
__device__ __forceinline__ void b6_ms_impl(
...
#define WAIT_ON_MMA(slot_)                                                              \
    ...
    // ── Main K-iter loop (multi-stage prefetch, 1 MMA in flight) ────────────
    //
    //   Pipeline structure mirrors hopper/_matmul_h1_ms.cu (ported from
    //   mma.sync to tcgen05.mma).  At any moment up to NS-1 TMA loads are
    //   in flight; the per-iteration MMA still completes before the next
    //   MMA is issued (1 in flight).  The load/compute overlap is the
    //   speedup vs b5_tma.
    //
    //   Per-iteration dependency graph:
    //       LOAD_TILE(nxt, ...)         ← issue async load of next K-tile
    //                ↓                    (runs concurrently with MMA below)
    //       WAIT_ON_TILE_READY(cur)     ← wait for current K-tile's load
    //                ↓
    //       MMA_ASYNC(cur, ...)         ← issue K_MMAS for current K-tile
    //                ↓
    //       WAIT_ON_MMA(cur)            ← block until MMA done (1 in flight)
    //
    const int num_tiles = K / BLOCK_K;

    // Prologue — fill the pipeline with NS-1 loads in flight.
    #pragma unroll
    for (int s = 0; s < NUM_STAGES - 1; s++) {
        LOAD_TILE(s, s * BLOCK_K);
    }

    // Main loop — each iter issues the next load (NS-1 ahead), then computes
    // the current K-tile.  num_tiles - (NS - 1) iters: the last NS-1 tiles
    // are drained below without issuing new loads.
    for (int k = 0; k < num_tiles - (NUM_STAGES - 1); k++) {
        const int cur = k % NUM_STAGES;
        const int nxt = (k + NUM_STAGES - 1) % NUM_STAGES;

        LOAD_TILE(nxt, (k + NUM_STAGES - 1) * BLOCK_K);
        WAIT_ON_TILE_READY(cur);
        MMA_ASYNC(cur, /*first_iter=*/(k == 0));
        WAIT_ON_MMA(cur);
    }

    // Drain — process the final NS-1 tiles (no new loads).
    #pragma unroll
...
void matmul_b6_ms_bm128_bn##BN_##_bk##BK_##_ns##NS_(                                \
    const __grid_constant__ CUtensorMap A_tmap,                                     \
    const __grid_constant__ CUtensorMap B_tmap,                                     \
    __nv_bfloat16* C_ptr, int M, int N, int K)                                      \
{                                                                                    \
    b6_ms_impl<BN_, BK_, NS_>(&A_tmap, &B_tmap, C_ptr, M, N, K);                    \
}
