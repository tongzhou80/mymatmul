...
_CU_PATH = os.path.join(_GPU_DIR, "_matmul_b7_msa.cu")
_CUBIN   = os.path.join(_GPU_DIR, f"_matmul_b7_msa_{SM_ARCH}.cubin")
...
def _kname(bn, bk, ns):
    return f"matmul_b7_msa_bm{BM}_bn{bn}_bk{bk}_ns{ns}"
...
def matmul_b7_msa(A, B):
    M, K = A.shape
    _, N = B.shape
    key = (M, N, K)
    if key not in _best:
        cfgs = [c for c in _CONFIGS if _legal_for_problem(c, M, N, K)]
        print(f"[b7_msa] autotuning {M}x{N}x{K} over {len(cfgs)} configs ...")
        _best[key] = _tune(M, N, K)
        bn, bk, ns = _best[key]
        print(f"[b7_msa] best: BN={bn} BK={bk} NS={ns}")
    bn, bk, ns = _best[key]
