...
Pairs two consecutive N-tiles and loads both with a single ldmatrix.x4.trans,
halving the B ldmatrix instruction count vs TC2b.
"""
...
def _kname(bm, bn, bk, nw):
    return f"matmul_cuda_tc4_bm{bm}_bn{bn}_bk{bk}_nw{nw}"
...
def matmul_tc4(A, B):
    ...
    if key not in _best:
        ...
        print(f"[tc4] best: BM={bm} BN={bn} BK={bk} NW={nw}")
