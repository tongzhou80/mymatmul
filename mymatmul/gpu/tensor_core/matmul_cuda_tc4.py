"""TC4: TC3 + CTA swizzling (GROUP_M) for improved L2 cache reuse.

Adds SWIZZLE (= GROUP_M) as a tunable parameter on top of TC3's PAD_A/PAD_B.
SWIZZLE=1 is the identity (no remapping, identical to TC3).
...
def _kname(bm, bn, bk, nw, pa, pb, sw):
    return f"matmul_cuda_tc4_bm{bm}_bn{bn}_bk{bk}_nw{nw}_pa{pa}_pb{pb}_sw{sw}"
...
def matmul_tc4(A, B):
    ...
    if key not in _best:
        cfgs = [c for c in _CONFIGS if M % c[0] == 0 and N % c[1] == 0 and K % c[2] == 0]
        print(f"[tc4] autotuning {M}x{K}x{N} over {len(cfgs)} configs ...")
        _best[key] = _tune(M, N, K)
        bm, bn, bk, nw, pa, pb, sw = _best[key]
        print(f"[tc4] best: BM={bm} BN={bn} BK={bk} NW={nw} PA={pa} PB={pb} SW={sw}")
