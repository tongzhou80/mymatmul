...
/*
 * TC4: TC2b with ldmatrix.x4.trans for B fragments.
 *
 * TC2b loads each 16×8 B tile with ldmatrix.x2.trans (2 regs, 16 threads used).
 * TC4 pairs two consecutive N-tiles (nt, nt+1) and loads both with a single
 * ldmatrix.x4.trans (4 regs, all 32 threads used), halving the ldmatrix
 * instruction count for B.
 *
 * Thread routing for x4.trans: each thread provides one row pointer.
 *   lane 0-15  (lane/16 == 0) → addresses tile nt
 *   lane 16-31 (lane/16 == 1) → addresses tile nt+1
 * Row within tile: _br = kk*16 + (lane%16), identical to TC2b.
 * XOR swizzle is applied per-tile before the address is computed, so no
 * layout changes are needed — only the B loading loop changes.
 *
 * Everything else (smem layout, XOR swizzle, double-buffer pipeline,
 * accumulator store) is identical to TC2b.
 */
...
__device__ __forceinline__ void matmul_tc4_impl(
    ...
    constexpr int WN_TILES = WARP_TILE_N / 8;   // must be even
...
// ── Compute tile: A uses ldmatrix.x4, B uses ldmatrix.x4.trans in pairs ──────
//
// B loading change vs TC2b:
//   Old: WN_TILES × ldmatrix.x2.trans  (2 regs, uses lanes 0-15 for both tiles)
//   New: (WN_TILES/2) × ldmatrix.x4.trans (4 regs, all 32 lanes)
//        Lanes 0-15  (lane/16 == 0) → B tile nt
//        Lanes 16-31 (lane/16 == 1) → B tile nt+1
//        Same _br formula as TC2b; _nc selects the appropriate tile.
    ...
    do {                                                                            \
        ...
        for (int _kk = 0; _kk < BK / 16; _kk++) {                                 \
            ...
            for (int _nt = 0; _nt < WN_TILES; _nt += 2) {                         \
                ...
                const int _ntl = _nt + (lane / 16);  /* nt for lanes<16, nt+1 for lanes>=16 */ \
                const int _nc  = warp_col * WARP_TILE_N + _ntl * 8;               \
...
void matmul_cuda_tc4_bm##BM_##_bn##BN_##_bk##BK_##_nw##NW_(                        \
    const __nv_bfloat16* __restrict__ A, const __nv_bfloat16* __restrict__ B,       \
    __nv_bfloat16* __restrict__ C, int M, int K, int N) {                           \
    matmul_tc4_impl<BM_, BN_, BK_, NW_>(A, B, C, M, K, N);                         \
}
