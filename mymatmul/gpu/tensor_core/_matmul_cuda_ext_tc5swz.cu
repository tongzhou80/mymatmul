 ...
 *
 * The GROUP_M remapping (same as tc2b_swz / Triton GROUP_M):
 *
 *   pid     = blockIdx.y * grid_n + blockIdx.x          (linear CTA ID)
 *   width   = GROUP_M * grid_n
 *   gid     = pid / width                               (super-group index)
 *   gm      = min(grid_m - gid*GROUP_M, GROUP_M)        (M-tiles in group)
 *   tile_m  = gid*GROUP_M + (pid % width) % gm
 *   tile_n  = (pid % width) / gm
 *
 * SW=1  → identity (no swizzle, identical to TC5).
 * SW=2/4/8 → reuse B tile across 2/4/8 consecutive M-tiles.
 *
 * Everything else — smem layout, XOR swizzle, ISSUE_TILE, COMPUTE_TILE,
 * vectorized bfloat162 write-back — is identical to TC5.
 */
...
template <int BM, int BN, int BK, int NUM_WARPS, int SW>
__device__ __forceinline__ void matmul_tc5swz_impl(
    ...
    // ── GROUP_M CTA swizzle (assumes grid_m is a multiple of SW) ─────────────
    const int pid    = blockIdx.y * gridDim.x + blockIdx.x;
    const int width  = SW * gridDim.x;
    const int gid    = pid / width;
    const int tile_m = gid * SW + (pid % width) % SW;
    const int tile_n = (pid % width) / SW;
...
void matmul_cuda_tc5swz_bm##BM_##_bn##BN_##_bk##BK_##_nw##NW_##_sw##SW_(           \
    const __nv_bfloat16* __restrict__ A, const __nv_bfloat16* __restrict__ B,       \
    __nv_bfloat16* __restrict__ C, int M, int K, int N) {                           \
    matmul_tc5swz_impl<BM_, BN_, BK_, NW_, SW_>(A, B, C, M, K, N);                 \
}
