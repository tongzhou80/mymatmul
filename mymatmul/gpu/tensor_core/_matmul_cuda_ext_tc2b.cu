 ...
 *
 * Extends TC2 (B-only swizzle) by applying the same XOR pattern to A.
 *
 * A tile swizzle (new):
 *   physical_chunk = logical_chunk ^ ((row / A_SHIFT) % A_SWZ)
 *   where A_SWZ = BK/8, A_SHIFT = 64/BK  (= 4 for BK=16, = 2 for BK=32).
 *
 *   With row stride BK*2 bytes, bank conflicts repeat with period A_SHIFT rows.
 *   XOR key = (row / A_SHIFT) % A_SWZ assigns a distinct physical chunk to each
 *   conflict-group, spreading 16 consecutive k-rows across all available chunks:
 *
 *   BK=16 (A_SWZ=2, A_SHIFT=4): groups of 4 rows, 2 chunk values.
 *     Row 0-3: chunk 0 → banks {0-3, 8-11, 16-19, 24-27}
 *     Row 4-7: chunk 1 → banks {4-7, 12-15, 20-23, 28-31}  ← fills the gaps
 *     Row 8-11: chunk 0 → 2-way with rows 0-3 (minimum: only 2 chunks exist)
 *     Row 12-15: chunk 1 → 2-way with rows 4-7
 *   Result: 4-way → 2-way (theoretical minimum for 2 distinct chunks).
 *
 *   BK=32 (A_SWZ=4, A_SHIFT=2): groups of 2 rows, 4 chunk values.
 *     Rows 0-1: chunk 0, rows 2-3: chunk 1, rows 4-5: chunk 2, rows 6-7: chunk 3
 *     All 8 threads (kk half) land on distinct bank groups.
 *     Rows 8-15 → 2-way conflict with rows 0-7.
 *   Result: 8-way → 2-way (theoretical minimum for 4 distinct chunks).
 *
 * B tile swizzle (unchanged from TC2):
 *   physical_chunk = logical_chunk ^ (row % B_SWZ), B_SWZ = BN/8.
 *   Reduces 16-way to 2-way.
 *
 * Template: (BM, BN, BK, NUM_WARPS) — same search space as TC1/TC2.
 */
...
__device__ __forceinline__ void matmul_tc2b_impl(
    ...
    // A tile swizzle: A_SWZ = number of 8-bf16 chunks per row; A_SHIFT = bank-conflict period.
    constexpr int A_SWZ   = BK / 8;
    constexpr int A_SHIFT = 64 / BK;   // = 4 for BK=16, = 2 for BK=32

    // B tile swizzle: same as TC2.
    constexpr int B_SWZ = BN / 8;

    extern __shared__ __nv_bfloat16 smem[];
    // Both A and B stored with XOR-swizzled column layout; same total size as TC2.
...
// ── Async load: A and B both XOR-swizzled ────────────────────────────────────
// A swizzle (8-bf16 chunk granularity):
//   physical_col = ((c/8) ^ ((r/A_SHIFT) % A_SWZ)) * 8 + c%8
// B swizzle (same as TC2):
//   physical_col = ((c/8) ^ (r % B_SWZ)) * 8 + c%8
...
// A address: logical chunk = kk*2 + (lane/16); un-swizzle with same formula.
//   physical_chunk = logical_chunk ^ ((row / A_SHIFT) % A_SWZ)
// B address: un-swizzle same as TC2.
...
void matmul_cuda_tc2b_bm##BM_##_bn##BN_##_bk##BK_##_nw##NW_(                       \
    const __nv_bfloat16* __restrict__ A, const __nv_bfloat16* __restrict__ B,       \
    __nv_bfloat16* __restrict__ C, int M, int K, int N) {                           \
    matmul_tc2b_impl<BM_, BN_, BK_, NW_>(A, B, C, M, K, N);                        \
}
