...
__device__ __forceinline__ void matmul_tc3_padB_impl(
...
// ── WMMA compute — A uses BK stride (unpadded), B uses BNP (padded) ──────────
...
void matmul_cuda_tc3_padB_bm##BM_##_bn##BN_##_bk##BK_##_nw##NW_(                        \
    const __nv_bfloat16* __restrict__ A, const __nv_bfloat16* __restrict__ B,       \
    __nv_bfloat16* __restrict__ C, int M, int K, int N) {                           \
    matmul_tc3_padB_impl<BM_, BN_, BK_, NW_>(A, B, C, M, K, N);                         \
}
