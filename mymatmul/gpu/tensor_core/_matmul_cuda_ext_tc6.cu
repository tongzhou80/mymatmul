...
/*
 * TC6: TC5 with kk-loop unroll factor fixed at 2.
 *
 * TC5 (and TC2b) use _Pragma("unroll") which fully unrolls the kk loop
 * (BK/16 iterations). For BK=64 this keeps 4 iterations' A/B fragment
 * registers live simultaneously → 168 regs/thread.
 *
 * TC6 uses _Pragma("unroll 2"), limiting live fragment registers to 2
 * iterations at a time regardless of BK:
 *   BK=16 (1 iter):  same as full unroll, no change.
 *   BK=32 (2 iters): same as full unroll, no change.
 *   BK=64 (4 iters): 2-way unroll → ~128 regs/thread (same as BK=32).
 *
 * This lets BK=64 fit the same occupancy profile as BK=32 while processing
 * twice as many K-elements per smem tile (higher arithmetic intensity).
 * Write-back is identical to TC5 (vectorized __nv_bfloat162 stores).
 */
