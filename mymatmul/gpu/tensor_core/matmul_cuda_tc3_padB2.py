...
_EXT = "_matmul_cuda_ext_tc3_padB2"
...
def _kname(bm, bn, bk, nw):
    return f"matmul_cuda_tc3_padB2_bm{bm}_bn{bn}_bk{bk}_nw{nw}"
...
def matmul_tc3_padB2(A, B):
    M, K = A.shape
    _, N = B.shape
    key  = (M, N, K)
    if key not in _best:
        cfgs = [c for c in _CONFIGS if M % c[0] == 0 and N % c[1] == 0 and K % c[2] == 0]
        print(f"[tc3_padB2] autotuning {M}x{K}x{N} over {len(cfgs)} configs ...")
        _best[key] = _tune(M, N, K)
        bm, bn, bk, nw = _best[key]
        print(f"[tc3_padB2] best: BM={bm} BN={bn} BK={bk} NW={nw}")
