...
/*
 * TC5: TC2b with vectorized write-back using __nv_bfloat162 stores.
 *
 * The mma.m16n8k16 output layout places each thread's 4 accumulators as:
 *   acc[0]: row=base_row,   col=base_col        (base_col = (lane%4)*2, always even)
 *   acc[1]: row=base_row,   col=base_col+1      <- consecutive with acc[0]
 *   acc[2]: row=base_row+8, col=base_col
 *   acc[3]: row=base_row+8, col=base_col+1      <- consecutive with acc[2]
 *
 * TC2b writes 4 individual __nv_bfloat16 values per (mt, nt).
 * TC5 packs each pair into one __nv_bfloat162 (32-bit) store, halving the
 * store instruction count: 2 stores per (mt, nt) instead of 4.
 *
 * Alignment: base_col is always even, so &C[gr*N + gc] is always 32-bit
 * aligned (given BN >= 8 and N a multiple of BN). The stores are safe.
 *
 * Everything else (smem layout, XOR swizzle, pipeline, ldmatrix) is
 * identical to TC2b.
 */
...
#undef COMPUTE_TILE
    ...
    // Vectorized write-back: pack consecutive column pairs into __nv_bfloat162.
    // acc[mt][nt][0,1] → same row, cols gc and gc+1 → one 32-bit store.
    // acc[mt][nt][2,3] → row+8,    cols gc and gc+1 → one 32-bit store.
