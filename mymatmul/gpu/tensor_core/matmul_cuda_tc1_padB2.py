"""TC1_padB2: BF16 WMMA matmul — TC1 with B-tile smem padding (PAD_B=8) + outer-product compute.

Same as TC1_padB but COMPUTE_TILE uses outer-product order: for each k-step,
all A fragments are loaded first, then all B fragments, then the full mma grid.
This loads each B fragment once per k-step rather than WM_TILES times.

Smem: (2*BM*BK + 2*BK*(BN+8))*2 bytes.
"""
...
_EXT = "_matmul_cuda_ext_tc1_padB2"
...
def _kname(bm, bn, bk, nw):
    return f"matmul_cuda_tc1_padB2_bm{bm}_bn{bn}_bk{bk}_nw{nw}"
...
def matmul_tc1_padB2(A, B):
    M, K = A.shape
    _, N = B.shape
    key  = (M, N, K)
    if key not in _best:
        cfgs = [c for c in _CONFIGS if M % c[0] == 0 and N % c[1] == 0 and K % c[2] == 0]
        print(f"[tc1_padB2] autotuning {M}x{K}x{N} over {len(cfgs)} configs ...")
        _best[key] = _tune(M, N, K)
        bm, bn, bk, nw = _best[key]
        print(f"[tc1_padB2] best: BM={bm} BN={bn} BK={bk} NW={nw}")
